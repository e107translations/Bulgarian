<?php
/*
 * e107 website system - Bulgarian Translation
 *
 * Copyright (C) 2005-2016 e107 Bulgaria github.com/e107translations/Bulgarian
 * Released under the terms and conditions of the
 * GNU General Public License (http://www.gnu.org/licenses/gpl.txt)
 *
 * Language File
 *
*/

define("COMLAN_0", "[забранено от администратора]");
define("COMLAN_1", "Деблокирай");
define("COMLAN_2", "Блокирай");
//define("COMLAN_3", "Delete"); //new > LAN_DELETE
define("COMLAN_4", "Инфо");
define("COMLAN_5", "Коментари ...");
define("COMLAN_6", "Трябва да влезете в профила си, за да публикувате коментар на този сайт - ако не сте регистриран потребител, кликнете");
define("COMLAN_7", "Главен администратор на сайта");
define("COMLAN_8", "Коментар");
define("COMLAN_9", "Публикувай коментар");
define("COMLAN_10", "Администратор");
define("COMLAN_11", "Записването на коментара беше неуспешно - моля пробвайте отново, като премахнете всички нестандартни знаци.");
define("COMLAN_12", "Потребител");
define("COMLAN_16", "Потребителско име:");
// define("COMLAN_99", "Comments");
define("COMLAN_100", "Новини");
define("COMLAN_101", "Анкета");
define("COMLAN_102", "Отговор до:");
define("COMLAN_103", "Статия");
define("COMLAN_104", "Рецензия");
define("COMLAN_105", "Съдържание");
define("COMLAN_106", "Файлове");
define("COMLAN_145", "Регистриран на:");
define("COMLAN_194", "Гост");
define("COMLAN_195", "Регистриран потребител");
define("COMLAN_310", "Невъзможно е поместването на коментара - използвате регистрирано потребителско име. Ако това е Вашето потребителско име, моля влезте и пробвайте отново.");
define("COMLAN_312", "Дублиране - записването е невъзможно.");
define("COMLAN_313", "Местоположение");
define("COMLAN_314", "редактирай коментара");
define("COMLAN_315", "Проследяване");
define("COMLAN_316", "Няма проследяване за тази новина.");
define("COMLAN_317", "Редактирай проследяване");
define("COMLAN_318", "Редакция на коментар");
define("COMLAN_319", "редактиран");
define("COMLAN_320", "Обновяване на коментара");
define("COMLAN_321", "тук");
define("COMLAN_322", "за регистрация");
define("COMLAN_323", "Грешка!");
define("COMLAN_324", "Относно");
define("COMLAN_325", "Ре:");
define("COMLAN_326", "Отговори на това");
//define("COMLAN_327", "Rating");//LAN_RATING
define("COMLAN_328", "Коментарите са спрени");
define("COMLAN_329", "Неауторизиран");
define("COMLAN_330", "IP:");
define("COMLAN_331", "В изчакване на одобрение!");
define("COMLAN_332", "Коментара не може да бъде изтрит");
define("COMLAN_333", "Коментара е одобрен");
define("COMLAN_334", "Коментара не може да бъде одобрен");
define("COMLAN_335", "Одобрен");
define("COMLAN_336", "Моля напишете нещо");
define("COMLAN_337", "Обновяването беше успешно");

define("COMLAN_400", "одобрен");
define("COMLAN_401", "блокиран");
define("COMLAN_402", "чакащ");

define("COMLAN_403", "Остави съобщение...");
define("COMLAN_404", "Одобряване");

define("COMLAN_TYPE_1", "новини");
define("COMLAN_TYPE_2", "файлове");
define("COMLAN_TYPE_3", "чзв");
define("COMLAN_TYPE_4", "анкета");
define("COMLAN_TYPE_5", "документи");
define("COMLAN_TYPE_6", "бъгове");
define("COMLAN_TYPE_7", "идеи");
define("COMLAN_TYPE_8", "потребителски профил");
define("COMLAN_TYPE_PAGE", "Съдържание");

define("COMLAN_500", "Моля [sign in] за да коментирате.");
define("COMLAN_501", "Ако все още не сте се регистрирали - [click here to register].");




?>